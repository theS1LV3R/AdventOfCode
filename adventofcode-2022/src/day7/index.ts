...
const lines = getLines<
  `$ cd ${string}` | "$ ls" | `${number | "dir"} ${string}`
>(import.meta.url, { filterEmpty: true, isTest: false });

type DirTree = Record<string, Record<string, number>>;
...
*/
function getDirSize(dirTree: DirTree, dir: string): number {
  const size = Object.values(dirTree[dir]).reduce((prev, cur) => prev + cur, 0);
  const subFolderSizes: number[] = [];

  const subFolders = Object.entries(dirTree).filter(
    ([subDir, val]) =>
      subDir.startsWith(dir === "/" ? dir : `${dir}/`) && subDir !== dir
  );

  if (subFolders.length > 0) {
    subFolders.forEach(([dir, _]) =>
      subFolderSizes.push(getDirSize(dirTree, dir))
    );
  }

  return size + subFolderSizes.reduce((prev, cur) => prev + cur, 0);
}

function part1(isTest = false) {
  let cwd = "";
    ...
    if (line.startsWith("$ ")) {
      ...
      if (cmd[0] === "cd") {
        if (cmd[1].startsWith("/")) {
          ...
          cwd = cwd === "/" ? `/${cmd[1]}` : `${cwd}/${cmd[1]}`;
        }
      } else {
        // Encountered ls, do nothing
      }
    } else {
      const [typeOrSize, fileOrDirname] = line.split(" ") as [
        "dir" | `${number}`,
        string
      ];
      ...
      if (typeOrSize === "dir") {
        const newDirName =
          cwd === "/" ? `/${fileOrDirname}` : `${cwd}/${fileOrDirname}`;

        dirTree[newDirName] = {};
      } else {
        dirTree[cwd][fileOrDirname] = parseInt(typeOrSize);
      }
    }
  }

  const sizes = Object.keys(dirTree).map((dir) => ({
    dir,
    size: getDirSize(dirTree, dir),
  }));

  // we are getting the size, just sum up the size of the dirs where the size is < 100_000 and that is the answer
  let summedSize = 0;

  sizes
    .filter((dir) => dir.size <= 100000)
    .forEach((dir) => (summedSize += dir.size));

  if (!isTest) console.log(`Day 7.1: ${summedSize}`);
  return summedSize;
}

if (import.meta.main) part1();
